...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   // no -999
   TH1F *h18 = new TH1F("h18","h18",500,-3,+3);
   TH1F *h19 = new TH1F("h19","h19",500,-3,+3);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	std::cout << "jentry " << jentry << "/" << nentries << " Event " << EventNum << std::endl;
        ...
        // testing ProtonTime[p]=0 exact
        Double_t p1time = 0.0000001777;
	Double_t p2time = 0.0000001777;

	Double_t p1timeerr = 0.0;
	Double_t p2timeerr = 0.0;
	Int_t np1 = 0;
	Int_t np2 = 0;
	Int_t nGoodVertex = 0;
	...
	if(nLiteTracks == 6 && nGoodVertex == 1)
	 {
	    ...
	    if(np1 == 1 && np2 == 1)
	     ...
	     if((np1==1) && (np2==1) && (p1time != 0) && (p2time != 0)
		         && (p1time != -999) && (p2time != -999) && (nTracksOneVertex<= 1000))
	     {
     ...
     std::cout << "jentry " << jentry
	       ...
	       << " vtx_z " << PrimVertexZ[0]
 	       << " np1 " << np1
 	       << " np2 " << np2
 	       << " ntk1vtx " << nTracksOneVertex
 	       << " goodvtx " << nGoodVertex
 	       << " nLiteTracks " << nLiteTracks
	       << std::endl;
     
	     }// end of selection
	     //std::cout << " nLiteTracks = " << nLiteTracks << std::endl;

	     // test
	     if((np1==1) && (np2==1) && (p1time != 0) && (p2time != 0) && (nTracksOneVertex<= 1000))
	     {
		// filtering
		if(p1time != -999)
		    if(p2time != -999)
		   {
		    h18->Fill(p2time-p1time);
		   }
		// excluding exact 0
		if(p1time != 0.0000001777)
		   if(p2time != 0.0000001777)
		  {
		   h19->Fill(p2time-p1time);
		  }
	     }
   ...
   for (Int_t i=0; i<m*(m-1)*(m-2);i++)
    {
    Int_t a = gRandom->Uniform(0,m);
    Int_t b = gRandom->Uniform(0,m);
    Int_t c = gRandom->Uniform(0,m);
    /*
    std::cout << " a " << a
              << " b " << b
              << " c " << c
  ...
  // 2+1: ID#15:vtxz+(t1=t2), ID#16:(vtxz=t1)+t2, ID#17:(vtxz=t2)+t1
    ...
    std::cout << " n case 1+1+1 = " << nnn << std::endl;
    std::cout << " l case 2+1 = " << l << std::endl;
    std::cout << " ll case 2+1 = " << ll << std::endl;
    std::cout << " lll case 2+1 = " << lll << std::endl;
   ...
   */
   ...
   TFile *fx = new TFile("output_LowPU_wmass_lowjet_NoTrig_.root","RECREATE");
   h01->Write();
   h02->Write();
   h1->Write();
   h2->Write();
