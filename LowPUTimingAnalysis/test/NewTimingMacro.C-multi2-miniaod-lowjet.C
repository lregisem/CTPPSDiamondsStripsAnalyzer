...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	//	if(nLiteTracks == 2 && nGoodVertex == 1)
	//  	if(nLiteTracks == 2)
	//  {
  ...
  // 2+1: ID#15:vtxz+(t1=t2), ID#16:(vtxz=t1)+t2, ID#17:(vtxz=t2)+t1
    ...
    std::cout << " n used 1+1+1 = " << nnn << std::endl;
    std::cout << " l used 2+1 = " << l << std::endl;
    std::cout << " ll used 2+1 = " << ll << std::endl;
    std::cout << " lll used 2+1 = " << lll << std::endl;
   ...
   */
   ...
   TFile *fx = new TFile("output_LowPU_wmass_lowjet_.root","RECREATE");
   h01->Write();
   h02->Write();
   h1->Write();
   h2->Write();
